...
const AddJobTraitModal = ({
  ...
  useEffect(() => {
    if (isOpen && !loadingAppData && stats.length > 0) {
      setHasLoaded(false);
      // Auto-select first stat if available and no stat is currently selected
      if (!newJobTraitForm.statId) {
        setNewJobTraitForm(prev => ({ ...prev, statId: stats[0].id }));
      }
      setHasLoaded(true);
  ...
  }, [isOpen, loadingAppData, stats, newJobTraitForm.statId]);
  ...
  const resetForm = () => {
    setNewJobTraitForm({
      name: "",
      level: 1,
      statId: stats.length > 0 ? stats[0].id : null,
      value: 0,
    });
    setError(null);
  };
