...
const AddJobBaseStatModal = ({
  ...
  useEffect(() => {
    if (isOpen && !loadingAppData && stats.length > 0) {
      ...
      if (availableBaseStats.length === 1 && !newJobBaseStatForm.statId) {
        setNewJobBaseStatForm(prev => ({ ...prev, statId: availableBaseStats[0].id }));
      }
      setHasLoaded(true);
  ...
  }, [isOpen, loadingAppData, stats, availableBaseStats, newJobBaseStatForm.statId]);
