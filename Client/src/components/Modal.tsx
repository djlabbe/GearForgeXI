...
const Modal = ({ isOpen, onClose, children, size = "md", customWidth }: ModalProps) => {
  ...
  bg-white shadow-xl rounded-lg dark:bg-gray-800`}
        ...
        >
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-1 rounded-full text-gray-400 hover:text-gray-600 hover:bg-gray-100 dark:hover:text-gray-300 dark:hover:bg-gray-700 transition-colors cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <svg
              className="w-5 h-5"
              fill="none"
